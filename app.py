...
from sqlalchemy import inspect, func, select, delete
import logging
...
# Throughout the app, you will see mock up data utilized in the postgres db, this mock up
# data has now been commented out and is not being used.
...
# Connects to a local database using config file.
...
# Moved models to models.py. Separation of concerns.
...
def venues():
  ...
  for v in Venue.query.all():
    if not f"{v.city};{v.state}" in locations:
      locations[f"{v.city};{v.state}"] = f"{v.id}"
    else:
      locations[f"{v.city};{v.state}"] += f",{v.id}"
  ...
  for location in locations:
    ...
    for venue_id in locations[location].split(','):
  ...
  print(locations)
  #print(list(locations.keys()).sort())
  return render_template('pages/venues.html', areas=data);

@app.route('/venues/search', methods=['POST'])
def search_venues():
  # Using func.lower to turn all rows in column name to lower case.
  venues = Venue.query.filter(func.lower(Venue.name).contains(str(request.form["search_term"]).lower()))
...
def show_venue(venue_id):
  ...
  now = datetime.now()
  # Running through is of the shows this venue is being referenced from.
  for show in venue.shows:
    date = datetime.strptime(show.start_time, '%Y-%m-%d %H:%M:%S')
    show_info = {"artist_id": show.artist.id,
      "artist_name": show.artist.name,
      "artist_image_link": show.artist.image_link,
      "start_time": show.start_time
    }

    # Deciding if its a past or upcoming show.
    if now > date:
      past_shows.append(show_info)
    else:
      upcoming_shows.append(show_info)

  # Setting up the data to be passed to the page.
  data = {
    "id": venue.id,
    "name": venue.name,
    "genres": venue.genres,
...
def create_venue_submission():
  ...
  try:
    ...
    newVenue.website = request.form['website_link']
...
def delete_venue(venue_id):
  try:
    ...
    db.session.rollback()
    flash("There was an error deleting venue " + venue_id)
    print(e)

  finally:
    db.session.close()

  flash("Venue " + venue_id + " was succesfully deleted.")
  return {'Success':True}
...
def search_artists():
  artists = Artist.query.filter(func.lower(Artist.name).contains(str(request.form["search_term"]).lower()))

  data = []

  # Place holder for artists found.
  response = {
    "count": artists.count(),
    "data": data
  }
...
def show_artist(artist_id):
  ...
  for show in artist.shows:
    date = datetime.strptime(show.start_time, '%Y-%m-%d %H:%M:%S')
    show_info = {"venue_id": show.venue.id,
...
def edit_artist(artist_id):
  ...
  # Populating form data with artist data from id.
...
def create_artist_submission():
  ...
  try:
    ...
    newArtist.website = request.form['website_link']
...
def create_show_submission():
  try:
   ...
   #db.session.execute(query)
    ...
    # Flush gives us the opportunity to reference the last added item to the db.
    db.session.add(show)
    ...
    print(e)
    db.session.rollback()
    flash('There was an error listing the show.\nMake sure you are entering an existing id for both Artist and Venue, and that the date has the correct format.')
